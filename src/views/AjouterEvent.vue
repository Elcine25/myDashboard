<template>  
  ...
  <div id="content">
      ...
      <NaavBar />
      <!-- End of Topbar -->
      <h3 class="mx-3">Ajouter un événement</h3>
  ...
  <div class="row">
      ...
      <label for="Ville" class="form-label col-12 ">
          <select class="form-control" id="Ville" name="villes_id" v-model="model.evenements.villes_id" >
            <option v-for="ville in villes" :value="ville.id" :key="ville.id">{{ ville.name }}</option>
  </select>
      </label>
  </div> 
  ...
  <div class="col">
   ...
   <br> 
    ...
    <button @click="saveEvent" class="btn-custom-color border  m-3 rounded p-2 fas fa-save">  Enregistrer</button>
</div>
</div>
 ...
 saveEvent() {
   ...
   const selectedCategorieId = this.model.evenements.categories_id;
    ...
    this.model.evenements.villes_id = selectedVilleId;
     ...
     axios
     .post('http://localhost:8000/api/evenements', this.model.evenements)
     .then(res => {
 ...
 canceladd() {
     this.$router.push('/listeevents'); // Rediriger vers la liste des catégories
   },
   getVilles(){
